...
\noindent TSU is a hardware implementation of a unit to generate timestampings, guaranteeing single-cycle timestamping accuracy, which is necessary for reliably merging the coarse delay with the \say{phaseMM} phase shift during fine delay calculation \cite{wr-draft}.

\vspace{5mm}

\noindent A diagram of the structure of a WR timestamping unit (TSU) can be found in the appendix \say{B.3 Coarse delay measurement} of the \say{White Rabbit Specification: Draft for Comments} \cite{wr-draft} document.

\vspace{5mm}

\noindent The timestamping is triggered through a pulse.
Due to the fact the WR is an extension of a ethernet PTP protocol this pulse is generated when the Physical Coding Sublayer (PCS) detects a Start-of-Frame Delimiter (SFD) character in the incoming (RX timestamps) or outgoing (TX timestamps) data stream.
